...
sr_pylint_single_file() { check_pylintrc; check_apt_package "pylint";  pylint --rcfile /tmp/pylintrc --max-line-length 120 $1; }
sr_pylint_single_file_sorted() { sr_pylint_single_file $1 | sort -V | grep -v "\*\*\*\*\*\*\*\*"; }
...
wait_for_filesize_bits() {
  ...
  while ! [ -f $FILENAME ]; do
    echo -en "${C_RET_CLR}Waiting for $FILENAME to exist... $(date)"
    sleep 1
  done
